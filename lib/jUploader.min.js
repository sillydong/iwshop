...
(function($) {
    window.jUploader = $.jUploader = function(options) {
        ...
        var initButton = function() {
            ...
            options.button.css({cursor: 'pointer', position: 'relative', overflow: 'hidden', direction: 'ltr'}).addClass('jUploader-button').bind('mouseover', function() {
        ...
        var createForm = function() {
            ...
            var form = $('<form id="' + id + '" name="' + id + '" action="../../scripts/lib/' + options.action + '" target="jUploaderIframe' + options.id + '" method="post" enctype="multipart/form-data" style="display:none"></form>');
            return form;
        };
        var createInput = function() {
            var input = $('<input type="file" onchange="this.blur();" accept="image/*" />');
            if (options.accept !== undefined) {
                input = $('<input type="file" onchange="this.blur();" accept="' + options.accept + '" />');
            }
            input.attr("id", 'jUploader-file' + options.id).attr("name", 'jUploaderFile').css({position: 'absolute', right: 0, top: 0, margin: 0, width: 'auto', opacity: 0, padding: 0, fontFamily: 'Arial', fontSize: '500px', verticalAlign: 'baseline', cursor: 'pointer', zIndex: 999}).bind('change', function() {
        ...
        var complete = function() {
            ...
            options.onComplete(options.fileName, response, innerHtml);
        };
    ...
    $.jUploader.defaults = {button: null, action: 'upload.aspx', allowedExtensions: [], cancelable: true, onUpload: function(fileName) {
        }, onComplete: function(fileName, response) {
        }, onCancel: function(fileName) {
        }, messages: {upload: 'Upload', cancel: 'Cancel', emptyFile: "{file} is empty, please select files again without it.", invalidExtension: "{file} has invalid extension. Only {extensions} are allowed.", onLeave: "The files are being uploaded, if you leave now the upload will be cancelled."}, showMessage: function(message) {
            alert(message);
        }, debug: false};
